...
def main():
    print(f"{'-'*100}\n{'-'*100}\n{'-'*100}\nTRAINGING...\n\nStart time:\t\t{datetime.now()}\n"
          f"Model name:\t\t{params['model']['name']}\n"
          f"Model architect:\t{params['model']['architect']}\n"
          f"Model package:\t\t{params['model']['tool']}\n"
          )
...
def tf_train():
    ...
    with tf.Session() as sess:
        ...
        if os.path.isfile(os.path.join(OUTPUT_PATH, params['model']['name'], 'TPRs')):  # keep previous learning curves
            TPRs = pickle.load(open(os.path.join(OUTPUT_PATH, params['model']['name'], 'TPRs'), 'rb'))
            PPVs = pickle.load(open(os.path.join(OUTPUT_PATH, params['model']['name'], 'PPVs'), 'rb'))
        ...
        for epoch in range(last_epoch, last_epoch + params['train']['epochs']):
            ...
            saver.save(sess, os.path.join(MODEL_PATH, params['model']['name'], params['model']['name']), global_step=epoch+1, write_meta_graph=False)
        ...
        with open(os.path.join(OUTPUT_PATH, params['model']['name'], 'TPRs'), 'wb') as pickle_file:
            pickle.dump(TPRs, pickle_file, pickle.HIGHEST_PROTOCOL)
        with open(os.path.join(OUTPUT_PATH, params['model']['name'], 'PPVs'), 'wb') as pickle_file:
...
def torch_train():
    ...
    # dirs
    if not os.path.isdir(os.path.join(MODEL_PATH, params['model']['name'])):
        os.makedirs(os.path.join(MODEL_PATH, params['model']['name']))
    ...
    train_ds = datasets.ImageFolder(root=TRAIN_PATH, transform=augmentator.pytorch_aumentator)
    ...
    # test data
    test_ds = datasets.ImageFolder(root=TEST_PATH, transform=augmentator.pytorch_aumentator)
    ...
    if params['model']['torch']['continue_learning']:  # continue learning from self-trained model at last point, load saved model first
        ...
        try:
            model = torch.load(os.path.join(MODEL_PATH, params['model']['name'], params['model']['name']+'.last.pth'))
            train_losses = pickle.load(open(os.path.join(OUTPUT_PATH, params['model']['name'], 'train.losses'), 'rb'))
            valid_losses = pickle.load(open(os.path.join(OUTPUT_PATH, params['model']['name'], 'valid.losses'), 'rb'))
        except ValueError:
    ...
    isupdated = 0
    print('pretrained epochs: ', len(train_losses)) 
    for epoch in range(last_epoch, last_epoch + params['train']['epochs']):
        ...
        # save last model
        torch.save(model, os.path.join(MODEL_PATH, params['model']['name'], params['model']['name']+'.last.pth'))
        # save best model (lowest valid loss)
        if valid_loss < best_val_loss:
            isupdated, best_val_loss = 1, valid_loss
            torch.save(model, os.path.join(MODEL_PATH, params['model']['name'], params['model']['name']+'.best.pth'))
    # save training losses and plot it
    print('trained epochs: ', len(train_losses))
    if params['train']['epochs'] > 0:
        with open(os.path.join(OUTPUT_PATH, params['model']['name'], 'train.losses'), 'wb') as pickle_file:
            pickle.dump(train_losses, pickle_file, pickle.HIGHEST_PROTOCOL)
        with open(os.path.join(OUTPUT_PATH, params['model']['name'], 'valid.losses'), 'wb') as pickle_file:
    ...
    if isupdated or params['train']['force_plot']:
        best_model = torch.load(os.path.join(MODEL_PATH, params['model']['name'], params['model']['name']+'.best.pth'))
